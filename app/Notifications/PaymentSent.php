...
use NotificationChannels\AfricaTalking\AfricaTalkingChannel;
use NotificationChannels\AfricaTalking\AfricaTalkingSmsMessage;
...
class PaymentSent extends Notification implements ShouldQueue
{
	...
	public function via($notifiable)
	{
		...
		if ($smsNotificationCanBeSent) {
			...
			if (config('settings.sms.driver') == 'africa_talking') {
				return [TwilioChannel::class];
			}
	...
	public function toAfricaTalking($notifiable)
	{
		return (new TwilioSmsMessage())->content($this->smsMessage());
	}
